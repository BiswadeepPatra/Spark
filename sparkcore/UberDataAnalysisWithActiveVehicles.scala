...
/** Compute the days on which each basement has more number of active vehicles. */

object UberDataAnalysisWithActiveVehicles {

	val format = new java.text.SimpleDateFormat("MM/dd/yyyy");
	val days = Array("Sun","Mon","Tue","Wed","Thu","Fri","Sat");

	/** Convert input data to (basement,Day,Vehicle count) tuples */
	def extractUderData(line: String) = {
		    ...
		    var basementWithDay = basement + ","+days(date.getDay()) 
				...
				val vehicles = fields(2)
				println("************************" + vehicles.toInt)		
				(basementWithDay,vehicles.toInt)
	}
